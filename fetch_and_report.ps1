...
document.addEventListener('mouseover',function(e){
  // Sub-popups
  var m=e.target.closest('.title-main');document.querySelectorAll('.sub-popup').forEach(function(p){p.style.display='none'});
  if(m){var pu=m.querySelector('.sub-popup');if(pu){var s=m.getAttribute('data-subs');if(s){if(pu.dataset.loaded!=='1'){var h='';s.split('||').forEach(function(x){var i=x.indexOf('|');if(i>0)h+="<a class='sub-item' href='"+x.substring(i+1)+"' target='_blank'>"+x.substring(0,i)+"</a>"});pu.innerHTML=h;pu.dataset.loaded='1'}pu.style.display='block';var r=m.getBoundingClientRect();pu.style.left=Math.min(r.left,window.innerWidth-pu.offsetWidth-10)+'px';pu.style.top=(r.bottom+4)+'px'}}}
  // K-line chart
  var td=e.target.closest('td.name');if(!td&&!e.target.closest('#klineTip')){kt.style.display='none';return}if(!td)return;
  var kd=td.getAttribute('data-kline');if(!kd||kd.length<10)return;
  var bars=kd.split('|').map(function(s){var p=s.split(',');return{o:+p[0],h:+p[1],l:+p[2],c:+p[3]}});var allP=[];for(var i=0;i<bars.length;i++){allP.push(bars[i].h,bars[i].l)}
  var min=Math.min.apply(null,allP),max=Math.max.apply(null,allP),range=max-min||1;var w=200,h=55,pt=3,pb=3,pl=2,bw=(w-pl-2)/bars.length,hw=Math.max(1,bw*0.6);var sy=function(v){return pt+(1-(v-min)/range)*(h-pt-pb)};var bodyW=Math.max(1,Math.min(hw-2,bw*0.75));var html='';for(i=0;i<bars.length;i++){var b=bars[i],cx=pl+i*bw+bw/2;var yh=sy(b.h),yl=sy(b.l),yo=sy(b.o),yc=sy(b.c);html+='<line x1="'+cx+'" y1="'+yh+'" x2="'+cx+'" y2="'+yl+'" stroke="#333" stroke-width="0.8"/>';var topY=Math.min(yo,yc),botY=Math.max(yo,yc);if(b.c>=b.o){html+='<rect x="'+(cx-bodyW/2)+'" y="'+topY+'" width="'+bodyW+'" height="'+(botY-topY||1)+'" fill="#fff" stroke="#333" stroke-width="0.6"/>'}else{html+='<rect x="'+(cx-bodyW/2)+'" y="'+topY+'" width="'+bodyW+'" height="'+(botY-topY||1)+'" fill="#000"/>'}}
  ks.setAttribute('viewBox','0 0 '+w+' '+h);ks.innerHTML=html;kh.textContent='H:'+max.toFixed(2);kl.textContent='L:'+min.toFixed(2)+' C:'+bars[bars.length-1].c.toFixed(2);kt.style.display='block';var r=td.getBoundingClientRect();kt.style.left=Math.min(Math.max(10,r.left),window.innerWidth-kt.offsetWidth-20)+'px';kt.style.top=(r.bottom+6)+'px'
});
</script>
</body>
</html>
